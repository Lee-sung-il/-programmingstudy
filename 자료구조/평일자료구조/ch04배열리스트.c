...
void Insert(int* arr, int data)
{
	...
	//꽉찬상태여부 확인(배열의 마지막 전 idx(98) 가 0(NULL)인지 확인)
...
{
	//순찬탐색
	//0 idx부터 차례로검색(Endlen()를 통해서 저장된 값의 길이를 확인한 후 반복)
	//idx요소와 target과 비교해서 일치하면 idx리턴
	//불일치시 -1리턴
	int len = Endlen(arr); //저장된 값의 길이
	for (int i = 0; i < len; i++)
	{
		if (arr[i] == target) {
			return i;
...
void DSearch(int* arr, int target)
{
	...
	11	11	22	22	33
	조회값 입력 : 22
	22는 1,2 idx에 총 2회중복되어있습니다.
	*/

	int cnt = 0; // 중복된 횟수를 저장할 용도
	...
	if (cnt != 0)// cnt != 0 -> target과 일치하는데이터 1이상
	{
		printf("\b idx에 총 %d번 반복되었습니다\n", cnt);
	}
	else {// cnt == 0 -> target과 일치하는데이터 X
...
int Remove(int* arr, int target)
{
	...
	// 2 해당 배열 위치의 요소 뒤에 있는 모든 값들을 삭제위치로 한칸씩 이동해서 덮어쓰기형태로 저장
	// 3 삭제할 데이터가 없다면 " 해당 데이터는 없는다\n" 출력
	int result = Search(arr, target); // -1 : 조회된데이터 x or 값에 해당 idx를 반환
